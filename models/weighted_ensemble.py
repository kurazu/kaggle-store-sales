...
def build_model(models: List[tf.keras.Model]) -> tf.keras.Model:
    ...
    ensemble_model.compile(
        ...
        optimizer=tf.keras.optimizers.Adam(
            learning_rate=0.001  # start with a different LR
        ),
        metrics=[
...
def build_and_train_model(
    *,
    ...
    # Figure out the most important comitee members
    # by fitting the ensemble head
    ...
    train_model(
        ...
        model_run_dir=os.path.join(model_run_dir, "ensemble"),
        training_config=training_config,
    )
    return model
